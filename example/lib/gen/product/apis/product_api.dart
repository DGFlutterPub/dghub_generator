...
class ProductApi {
  Future<Product> productStore({required FormData form}) async {
    try {
      var response =
          await ApiService.request().post('/product_store', data: form);
  ...
  Future<Product> productUpdate(
      {required String id, required FormData form}) async {
    try {
      var response =
          await ApiService.request().post('/product_update/$id', data: form);
      return Product.fromJson(response.data);
    } catch (e, s) {
      throw e.toString();
    }
  ...
  Future<Product> productDestroyForever({required String id}) async {
    try {
      var response =
          await ApiService.request().delete('/product_destroy_forever/$id');
      return Product.fromJson(response.data);
    } catch (e, s) {
      throw e.toString();
    }
