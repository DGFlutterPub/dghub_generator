...
import '../validator/dghub_validator_builder.dart';

class DGSchema extends SchemaValue {
  DGSchema.shape(
    this._schema, {
  ...
  Map<dynamic, dynamic> catchErrors(Map<dynamic, dynamic> form) {
    ...
    _schema.forEach((key, value) {
      if (value is DGValidator) {
        var error = value.build()(data[key], data);
        if (error != null) {
          errors[key] = error;
        }
      } else if (value is DGSchema) {
    ...
    if (noUnknown) {
      for (var key in form.keys) {
        if (!_schema.containsKey(key)) {
          errors[key] = DGValidator.globalLocale.unknownFieldMessage;
        }
      }
    }
