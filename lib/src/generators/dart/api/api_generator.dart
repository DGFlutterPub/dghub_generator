...
import 'package:dghub_generator/src/generators/dart/form/form_generator.dart';
import 'package:dghub_generator/src/generators/dart/provider/provider_generator.dart';
...
''');
      ...
      if (api.action == DGApiAction.destroyAll) {
        ...
        body.add('''
Future<bool> ${classPathName.toCamelCase()}() async {
    var response = await ApiService.request($baseOption).${api.method.name}('/${classPathName.snakeCase}');
    try {
...
''');
      ...
      if (api.action == DGApiAction.getOne) {
        ...
        body.add('''
Future<${className.toPascalCase()}> ${classPathName.toCamelCase()}({required String ${api.getOneKey}}) async {
   var response = await ApiService.request($baseOption).${api.method.name}('/${classPathName.snakeCase}/\$${api.getOneKey}');
    try {
      return ${className.toPascalCase()}.fromJson(response.data);
    } catch (e, s) {
      throw response.data.toString();
...
''');
    ...
    read = read
        ...
        .replaceAll('/*body*/', Tools.getNewLineString(body));
